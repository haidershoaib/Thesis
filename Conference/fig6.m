...
lambda = 1./lambda_bar;
tau = [0.0002, 0.0006 0.001]; % Processing time
...
Q2 = V_opt2.*lambda.*Y;
plot(mu, Q2, 'r', 'LineWidth', 1.2);
hold on 
% Y3 = exp(l(3).*lambda).*gammainc(lambda,l(3).*lambda,'upper');
Q3 = V_opt3.*lambda.*Y;
plot(mu, Q3, 'b', 'LineWidth', 1.2);
hold on;
plot(mu(1:4:end), Qsim1(1:4:end), 'ko')
hold on;
plot(mu(1:4:end), Qsim2(1:4:end), 'ro')
hold on;
plot(mu(1:4:end), Qsim3(1:4:end), 'bo')
hold off;
grid on
...
legend('\tau = 0.0002 s (Ana.)', '\tau = 0.0006 s (Ana.)', '\tau = 0.001 s (Ana.)', '\tau = 0.0002 s (Sim.)', '\tau = 0.0006 s (Sim.)', '\tau = 0.001 s (Sim.)');