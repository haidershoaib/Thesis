...
VehNum=2:10:1000; % number of vehicles in the simulation
del=0.1; %initial distance between vehicles
...
R_th = [0.5*10^9, 1*10^9, 1.2*10^9, 5*10^9];
for rth_val=1:length(R_th)
    for cav=2:length(VehNum)
        x=linspace((cav-1)*del,0,cav); %the initial location of the N vehicles
        V=ones(TimeStep,cav)*45*1000/3600; %initial speed of the vehicles
        a=zeros(TimeStep,cav);
        X = zeros(TimeStep, cav);
        A = zeros(TimeStep, cav);
        % Vector allocations for location of CAVs and dsitance from CAV to BS
        x_cav = zeros(TimeStep-1, cav);
        y_cav = zeros(TimeStep-1, cav);
        cavToBS = zeros(TimeStep-1, cav);
        ...
        l = 1;
        tau_proccess = 0.00015; %reaction of the drivers
            ...
            for i=1:cav
                ...
                if i==1
                    ...
                    if t>1
                        ...
                        V_data(t, i) = (1/(h_d*mu_opt))*(1 - (R_th(rth_val)./R_m(t, i)));
                        V_safe(t, i) = h;
        ...
        vAvg(rth_val,cav-1) = mean(mean(V(1:cav-1, :), 2));
        % disp(mean(vAvg))
        cav_density(rth_val,cav-1) = cav/perim;
        flow(rth_val,cav-1) = vAvg(rth_val,cav-1)*cav_density(rth_val,cav-1);
        %     disp(flow(cav-1))
    end
end
%% Animation
if anim==1
    ...
    plot(cav_density(1,:), flow(1,:), 'LineWidth', 1.2)
    hold on
    plot(cav_density(2,:), flow(2,:), 'LineWidth', 1.2)
    hold on
    plot(cav_density(3,:), flow(3,:), 'LineWidth', 1.2)
    hold on
    plot(cav_density(4,:), flow(4,:), 'LineWidth', 1.2)
    hold off
    ylabel('Traffic Flow (Q)')
    xlabel('CAV Density (k)')
    grid on;
    legend('R_{th} = 0.5 Gbps', 'R_{th} = 1 Gbps', 'R_{th} = 1.2 Gbps', 'R_{th} = 5 Gbps');
