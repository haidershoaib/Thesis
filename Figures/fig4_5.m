...
hold off;
grid on
xlabel('Average CAV Density (\lambda)');
ylabel('Traffic Flow (Q)');
% title('Q vs. 1/\lambda with Varying Data Rate Thresholds');
legend('R_{th} = 1 Gbps (Ana.)', 'R_{th} = 1.05 Gbps (Ana.)', 'R_{th} = 1.1 Gbps (Ana.)', 'R_{th} = 1 Gbps (Sim.)', 'R_{th} = 1.05 Gbps (Sim.)', 'R_{th} = 1.1 Gbps (Sim.)');
...
hold off;
xlabel('Average CAV Spacing (1/\lambda)');
